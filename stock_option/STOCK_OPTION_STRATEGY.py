# On 21/09/2022 changed SL exit and entry to live candle
...
logging.basicConfig(filename=r"C:\Users\Administrator\Desktop\NEW_STRATEGY\STOCK_OPTION\STOCK OPTION LOG.txt", level=logging.DEBUG, format="%(asctime)s;%(levelname)s;%(message)s",datefmt="%Y-%m-%d %H:%M:%S")
...
lines=[]
with open(r'C:\Users\Administrator\Desktop\NEW_STRATEGY\STOCK_OPTION\ZERODHA_USER_DATA.txt', 'r') as f:  # Open file for read
...
lines=[]
with open(r'C:\Users\Administrator\Desktop\NEW_STRATEGY\STOCK_OPTION\ALICE_USER_DATA.txt', 'r') as f:  # Open file for read
    for line in f:           # Read line-by-line
        lines.append( line.strip() )


alice_username = lines[0]
alice_password = lines[1]
alice_twoFA = lines[2]
alice_mpin = lines[3]
alice_apikey = lines[4]

STRATEGY_DATA = []
with open(r'C:\Users\Administrator\Desktop\NEW_STRATEGY\STOCK_OPTION\STRATEGY_DATA.csv') as csv_file :
...
alice = Aliceblue(user_id=alice_username,
                  api_key=alice_apikey)
logging.debug(alice.get_session_id())
logging.debug("Aliceblue login successful")
...
with open(r'C:\Users\Administrator\Desktop\NEW_STRATEGY\STOCK_OPTION\STOCK_LIST_DATA.csv') as csv_file :
    csv_reader = csv.reader(csv_file)
    for row in csv_reader:
        if len(row) > 0 :
...
for instrument in INPUT_SPOT_LIST :
    ...
    try :
        ins_token = int(ZERODHA_INSTRUMENT_DATA.loc[(ZERODHA_INSTRUMENT_DATA['name']==INPUT_SPOT_TO_SPOT[instrument])]['instrument_token'].iloc[0])
        trading_symbol = ZERODHA_INSTRUMENT_DATA.loc[(ZERODHA_INSTRUMENT_DATA['name']==INPUT_SPOT_TO_SPOT[instrument])]['tradingsymbol'].iloc[0]
    except Exception as e:  
        print("Skipping instrument {} due to its absence in Futures".format(instrument))
        logging.debug("Skipping instrument {} due to its absence in Futures".format(instrument))
        continue
    ...
    hist_data[trading_symbol] = kite.historical_data(InstrumentID[trading_symbol],start_date,end_date,"minute")
...
def PlaceEntryOrder(instrument,entry_type,price):
    ...
    logging.debug("ENTRY order in {} in {} DIRECTION , strike = {} , qty = {}".format(instrument,entry_type,STRIKE,qty))
    ...
    if LIVE == 1 :
        order_data = alice.place_order(transaction_type = TransactionType.Buy,
                                ...
                                trailing_sl = None,
                                is_amo = False)
        logging.debug("Order details {}".format(order_data))
    ...
    for instrument in INPUT_SPOT_LIST :
        if TRADE_TAKEN[instrument] !="None" :
            ...
            if TRADE_TAKEN[instrument] == "BUY" :
                ...
                #     TRADE_TAKEN[instrument] = "None"
                #     logging.debug("Exit of {} due to MBB".format(instrument))
                #     continue
                if Exit2 :
                    ExitStock(instrument)
                    TRADE_TAKEN[instrument] = "None"
                    logging.debug("Exit of {} due to SL HIT".format(instrument))
                    continue
                else :
                    if LIMIT_STORE[instrument] == -1 :
                        ...
                        if check1:
                            ...
                            logging.debug(("Storing mark value for crossing UBB of {} as {}".format(instrument,LIMIT_STORE[instrument])))
                    elif fiveMin_HA.iat[-2,fiveMin_col_mapper['low']] > LIMIT_STORE[instrument] :
                        LIMIT_STORE[instrument] = fiveMin_HA.iat[-2,fiveMin_col_mapper['low']]
                        logging.debug(("Updating mark value for having a higher low of {} as {}".format(instrument,LIMIT_STORE[instrument])))
                    elif fiveMin_HA.iat[-2,fiveMin_col_mapper['close']] < LIMIT_STORE[instrument] :
                        ExitStock(instrument)
                        TRADE_TAKEN[instrument] = "None"
                        logging.debug("Exit of {} due to UBB".format(instrument))
                        continue
                ...
                if Exit2 :
                    ...
                    if LIMIT_STORE[instrument] == -1 :
                        ...
                        if check1:
                            ...
                            logging.debug(("Storing mark value for crossing LBB of {} as {}".format(instrument,LIMIT_STORE[instrument])))
                    elif fiveMin_HA.iat[-2,fiveMin_col_mapper['high']] < LIMIT_STORE[instrument] :
                        LIMIT_STORE[instrument] = fiveMin_HA.iat[-2,fiveMin_col_mapper['high']]
                        logging.debug(("Updating mark value for having a lower high of {} as {}".format(instrument,LIMIT_STORE[instrument])))
                    elif fiveMin_HA.iat[-2,fiveMin_col_mapper['close']] > LIMIT_STORE[instrument] :
                        ExitStock(instrument)
                        TRADE_TAKEN[instrument] = "None"
                        logging.debug("Exit of {} due to LBB".format(instrument))
                        continue
...
for instrument in INPUT_SPOT_LIST :
    if TRADE_TAKEN[instrument] !="None" :
        ExitStock(instrument)
        TRADE_TAKEN[instrument] = "None"
        logging.debug("Exit of {} due to Day Exit".format(instrument))
